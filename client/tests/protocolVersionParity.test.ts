...
/**
 * `client/src/lib/protocolVersion.ts` and `relay/src/protocol/version.ts`
...
describe("the WS protocol version", () => {
  it("is identical on both sides", () => {
    expect(body("src/lib/protocolVersion.ts")).toBe(body("../relay/src/protocol/version.ts"));
  });

  it("has each copy point at the other", () => {
    expect(readFileSync(resolve(process.cwd(), "src/lib/protocolVersion.ts"), "utf8")).toContain(
      "relay/src/protocol/version.ts",
    );
    expect(readFileSync(resolve(process.cwd(), "../relay/src/protocol/version.ts"), "utf8")).toContain(
      "client/src/lib/protocolVersion.ts",
    );
  });
});

...
/**
 * `client/src/lib/theme/theme.ts` and `relay/src/host/theme.ts` are the same validator,
...
describe("the theme validator", () => {
  it("is identical on both sides", () => {
    expect(body("src/lib/theme/theme.ts")).toBe(body("../relay/src/host/theme.ts"));
  });
  ...
  it("has each copy point at the other", () => {
    ...
    expect(readFileSync(resolve(process.cwd(), "src/lib/theme/theme.ts"), "utf8")).toContain("relay/src/host/theme.ts");
    expect(readFileSync(resolve(process.cwd(), "../relay/src/host/theme.ts"), "utf8")).toContain("client/src/lib/theme/theme.ts");
  });
});

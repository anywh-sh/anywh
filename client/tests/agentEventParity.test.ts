...
/**
 * `client/src/lib/agent-event.ts` and `relay/src/protocol/agent-event.ts`
...
describe("the AgentEvent contract", () => {
  it("is identical on both sides", () => {
    expect(body("src/lib/agent-event.ts")).toBe(body("../relay/src/protocol/agent-event.ts"));
  });

  it("has each copy point at the other", () => {
    expect(readFileSync(resolve(process.cwd(), "src/lib/agent-event.ts"), "utf8")).toContain(

...
# inside WSL2.
#
# The relay half of the installer. `install.sh` is the front door and routes
# here — explicitly on `--relay-only`, implicitly whenever a profile flag
# appears — so the commands a human types stay the ones they always were:
#
...
#
# Reachable on its own at https://anywh.sh/install-relay, which is what the
# dispatcher downloads when it isn't running from a checkout or an extracted
# release tree. Nothing in here knows about the app, and it never should:
# this script's whole surface is the relay, its service and its profiles.
#
...
usage() {
  cat <<USAGE
Usage: install-relay.sh [--version <tag>] [--porcelain] [--mode dev|prod]
                        [--profile-id <id>] [--profile-label <text>]
                        [--relay-host <ip>|auto] [--profile-home <path>]
                        [--no-profile]
